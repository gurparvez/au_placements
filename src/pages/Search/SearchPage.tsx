...
const rowDivider = '1px solid var(--border)';
...
const SectionTitle: React.FC<{ icon: React.ReactNode; label: string; count: number }> = ({ icon, label, count }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0 2px 10px', color: 'var(--text-muted)', fontSize: 13.5, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.05em' }}>
    <span style={{ color: 'var(--brass)', display: 'inline-flex' }}>{icon}</span> {label} <span className="data" style={{ display: 'inline-block', padding: '0 8px', borderRadius: 999, background: 'var(--primary-soft)', color: 'var(--primary)', fontWeight: 600, fontSize: 12 }}>{count}</span>
  </div>
);
    ...
    <section style={{ padding: '20px clamp(20px,10vw,64px) 80px' }}>
      <div style={{ position: 'relative', marginBottom: 24 }}>
        ...
        <input
          ...
          style={{ width: '100%', padding: '13px 44px 13px 44px', border: '1px solid var(--border-strong)', borderRadius: 'var(--r-ctl)', background: 'var(--surface)', color: 'var(--text)', fontSize: 16, fontWeight: 500, outline: 'none', boxShadow: 'var(--shadow-sm)' }}
        />
        {loading && <Loader2 size={17} className="kp-spin" style={{ position: 'absolute', right: 14, top: 15, color: 'var(--text-subtle)' }} />}
      </div>

      {q.trim().length < 2 ? (
        <div style={{ ...card, padding: '48px 24px', textAlign: 'center', color: 'var(--text-muted)', fontSize: 15 }}>
          Type at least 2 characters to search.
        </div>
      ) : nothing ? (
        <div style={{ ...card, padding: '48px 24px', textAlign: 'center', color: 'var(--text-muted)' }}>
          <Search size={26} style={{ opacity: 0.5 }} />
          <p style={{ margin: '10px 0 0', fontSize: 15 }}>No people or companies match “{q.trim()}”.</p>
        </div>
      ) : (
        <Reveal>
                ...
                {people.map((r, i) => {
                  ...
                  return (
                    <Link
                      key={r._id}
                      to={`/profiles/${r._id}`}
                      style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', textDecoration: 'none', color: 'var(--text)', borderTop: i === 0 ? 'none' : rowDivider, transition: 'background .15s' }}
                      onMouseEnter={(e) => (e.currentTarget.style.background = 'var(--surface-2)')}
                      onMouseLeave={(e) => (e.currentTarget.style.background = 'transparent')}
                    >
                      <span aria-hidden style={{ width: 40, height: 40, borderRadius: '50%', flex: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: 15, background: avatarColor(name) }}>{initials(r.firstName, r.lastName) || '?'}</span>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: 650, fontSize: 15.5, textTransform: 'capitalize', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{name || 'Unnamed'}</div>
              ...
              <div style={card}>
                {companies.map((c, i) => (
                  <button
                    ...
                    style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', border: 'none', borderTop: i === 0 ? 'none' : rowDivider, background: 'transparent', cursor: 'pointer', textAlign: 'left', color: 'var(--text)', transition: 'background .15s' }}
                    onMouseEnter={(e) => (e.currentTarget.style.background = 'var(--surface-2)')}
                    onMouseLeave={(e) => (e.currentTarget.style.background = 'transparent')}
                  >
                    {c.logo ? (
                      ...
                      <span aria-hidden style={{ width: 40, height: 40, borderRadius: 10, flex: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: 15, background: avatarColor(c.company) }}>{companyInitials(c.company)}</span>
                    )}
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: 650, fontSize: 15.5, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.company}</div>
                      <div style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--text-muted)', display: 'flex', gap: 12, flexWrap: 'wrap' }}>
