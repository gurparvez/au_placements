...
const h2: React.CSSProperties = { fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 500, letterSpacing: '-.01em', margin: '0 0 16px' };
...
const Shell: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <section style={{ padding: '24px clamp(20px,10vw,64px) 80px' }}>
    <Link to="/students" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 14.5, color: 'var(--text-muted)', textDecoration: 'none', fontWeight: 500 }}>
...
const PublicStudentProfile: React.FC = () => {
    ...
    return (
      <Shell>
        <div style={{ marginTop: 40, textAlign: 'center', padding: '64px 24px', background: 'var(--surface)', border: '1px dashed var(--border-strong)', borderRadius: 'var(--r-card)' }}>
          <div aria-hidden style={{ width: 54, height: 54, borderRadius: '50%', background: 'var(--surface-2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 23, margin: '0 auto', color: 'var(--text-subtle)' }}>?</div>
          <h2 style={{ fontSize: 22, fontWeight: 700, margin: '18px 0 0' }}>Profile not found</h2>
          <p style={{ fontSize: 15.5, color: 'var(--text-muted)', margin: '8px 0 0' }}>Profile removed or link incorrect.</p>
          <Link to="/students" style={{ display: 'inline-flex', marginTop: 18, padding: '10px 18px', borderRadius: 'var(--r-ctl)', background: 'var(--primary)', color: 'var(--on-primary)', fontWeight: 600, fontSize: 15, textDecoration: 'none' }}>Back to register</Link>
        </div>
      </Shell>
    );
  }
  ...
  const connectButton = () => {
    ...
    const base: React.CSSProperties = { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '11px 18px', borderRadius: 'var(--r-ctl)', fontWeight: 600, fontSize: 15, cursor: 'pointer', border: '1px solid var(--border-strong)', background: 'var(--surface)', color: 'var(--text)' };
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 20, flexWrap: 'wrap' }}>
            <div style={{ flex: 1, minWidth: 260 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginTop: 12 }}>
                ...
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 13, fontWeight: 600, padding: '3px 10px', borderRadius: 'var(--r-pill)', background: 'var(--surface-2)', color: 'var(--brass)', border: '1px solid var(--brass-line)' }}><BadgeCheck size={13} /> {u.verified ? 'Verified' : 'On the register'}</span>
              </div>
              {p.headline && <p style={{ fontSize: 16.5, color: 'var(--text)', margin: '6px 0 0' }}>{p.headline}</p>}

              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 14, marginTop: 12, fontSize: 14.5, color: 'var(--text-muted)' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 14 }}>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13.5, fontWeight: 550, padding: '5px 12px', borderRadius: 'var(--r-pill)', background: 'var(--primary-soft)', color: 'var(--primary)' }}>
                ...
                {av && <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 13.5, fontWeight: 500, padding: '5px 12px', borderRadius: 'var(--r-pill)', background: 'var(--surface-2)', color: 'var(--text-muted)' }}><CalendarClock size={13} /> Available {av}</span>}
              </div>
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, minWidth: 170, marginTop: 16 }}>
              {isRecruiter ? (
                <button onClick={openCompose} {...primaryHover} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '11px 18px', borderRadius: 'var(--r-ctl)', background: 'var(--primary)', color: 'var(--on-primary)', fontWeight: 600, fontSize: 15, cursor: 'pointer', border: 'none', transition: 'background .18s ease' }}><Mail size={16} /> Contact</button>
              ) : (
                <a href={contactHref(cs)} {...primaryHover} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '11px 18px', borderRadius: 'var(--r-ctl)', background: 'var(--primary)', color: 'var(--on-primary)', fontWeight: 600, fontSize: 15, textDecoration: 'none', transition: 'background .18s ease' }}><Mail size={16} /> Contact</a>
              )}
              {connectButton()}
              {canMessage && (
                <button onClick={startMessage} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '11px 18px', borderRadius: 'var(--r-ctl)', background: 'var(--surface)', border: '1px solid var(--border-strong)', color: 'var(--text)', fontWeight: 600, fontSize: 15, cursor: 'pointer' }}><MessageCircle size={16} /> Message</button>
              )}
              {p.resume_link && <a href={p.resume_link} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '11px 18px', borderRadius: 'var(--r-ctl)', background: 'var(--surface)', border: '1px solid var(--border-strong)', color: 'var(--text)', fontWeight: 600, fontSize: 15, textDecoration: 'none' }}><FileText size={16} /> Résumé</a>}
            </div>
          </div>
        </div>
      </div>
      ...
      <div data-kp-split="true" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 320px', gap: 18, marginTop: 18, alignItems: 'start' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18, minWidth: 0 }}>
          {p.about?.trim() && (
            ...
            <Section title="About" icon={<UserIcon size={17} />}>
              <p style={{ fontSize: 15.5, color: 'var(--text-muted)', lineHeight: 1.7, margin: 0, whiteSpace: 'pre-line' }}>{p.about}</p>
            </Section>
            </Reveal>
          )}
                ...
                {p.experience.map((ex: any, i: number) => (
                  <div key={i} style={{ display: 'flex', gap: 14 }}>
                    ...
                    <div style={{ paddingBottom: 2 }}>
                      <div style={{ fontWeight: 650, fontSize: 16 }}>{ex.role}</div>
                      <div style={{ fontSize: 14.5, color: 'var(--text-muted)' }}>{ex.company} · {rangeYears(ex.start_date, ex.end_date, !ex.end_date)}</div>
                      {ex.description && <p style={{ fontSize: 14.5, color: 'var(--text-muted)', margin: '8px 0 0', lineHeight: 1.6 }}>{ex.description}</p>}
                    </div>
                  </div>
                ))}
              </div>
                ...
                {resolved.map((pr: any, i: number) => (
                  <div key={i} style={{ border: '1px solid var(--border)', borderRadius: 12, padding: 16, background: 'var(--bg-2)' }}>
                    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap' }}>
                      <span style={{ fontWeight: 650, fontSize: 16 }}>{pr.title}</span>
                      <span style={{ fontSize: 13.5, color: 'var(--text-subtle)' }}>{pr.on_going ? `${yearOf(pr.start_date)} – Ongoing` : rangeYears(pr.start_date, pr.end_date, false)}</span>
                    </div>
                    {pr.description && <p style={{ fontSize: 14.5, color: 'var(--text-muted)', margin: '8px 0 0', lineHeight: 1.6 }}>{pr.description}</p>}
                    {pr.tech_resolved?.length > 0 && (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 12 }}>
                        {pr.tech_resolved.map((t: string, j: number) => <span key={j} style={{ fontSize: 12.5, padding: '3px 8px', borderRadius: 'var(--r-chip)', background: 'var(--surface-2)', color: 'var(--text-muted)', border: '1px solid var(--border)' }}>{t}</span>)}
                      </div>
                    )}
                    <div style={{ display: 'flex', gap: 14, marginTop: 12 }}>
                      {pr.code_url && <a href={pr.code_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 14, fontWeight: 600, color: 'var(--primary)', textDecoration: 'none' }}>Code ↗</a>}
                      {pr.live_url && <a href={pr.live_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 14, fontWeight: 600, color: 'var(--primary)', textDecoration: 'none' }}>Live ↗</a>}
                    </div>
                  </div>
                ))}
              </div>
                  ...
                  return (
                    <div key={i}>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
                        {isSchool ? (
                          ed.grade && <span style={{ fontSize: 12.5, fontWeight: 550, padding: '3px 9px', borderRadius: 'var(--r-pill)', background: 'var(--surface-2)', color: 'var(--text-muted)' }}>School</span>
                        ) : (
                          <span style={{ fontSize: 12.5, fontWeight: 550, padding: '3px 9px', borderRadius: 'var(--r-pill)', background: 'var(--surface-2)', color: 'var(--text-muted)' }}>{CATEGORY[ed.course?.category] || 'Course'}</span>
                        )}
                        {!isSchool && ed.specialization && <span style={{ fontSize: 12.5, padding: '3px 9px', borderRadius: 'var(--r-pill)', background: 'var(--surface-2)', color: 'var(--text-muted)' }}>{ed.specialization}</span>}
                      </div>
                    </div>
                  );
                })}
                ...
                {p.certificates.map((c: any, i: number) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
                    <div>
                      ...
                      <div style={{ fontSize: 14, color: 'var(--text-muted)', marginTop: 2 }}>{c.issued_by} · {fmtMonth(c.issue_date)}</div>
                      {c.valid_until && <div style={{ fontSize: 13.5, color: 'var(--text-subtle)', marginTop: 2 }}>Valid until {fmtMonth(c.valid_until)}</div>}
                    </div>
                    {c.certificate_url && <a href={c.certificate_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 14, fontWeight: 600, color: 'var(--primary)', textDecoration: 'none', flex: 'none' }}>View ↗</a>}
                  </div>
                ))}
              </div>
            </Section>
          ...
          {(p.skills || []).length > 0 && (
            ...
            <Section title="Skills" icon={<Wrench size={17} />}>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
                {p.skills.map((s: any, i: number) => <span key={i} style={{ fontSize: 13.5, padding: '5px 11px', borderRadius: 'var(--r-pill)', background: 'var(--surface-2)', color: 'var(--text)', border: '1px solid var(--border)' }}>{s.displayName || s.name}</span>)}
              </div>
            </Section>
            </Reveal>
          )}
        ...
        <aside style={{ display: 'flex', flexDirection: 'column', gap: 18, position: 'sticky', top: 84 }}>
          <div style={{ ...card, padding: 22 }}>
            <h2 style={{ ...h2, fontSize: 17, margin: '0 0 14px' }}>Contact</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {[
                ...
                <a
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: 11, textDecoration: 'none', color: 'var(--text)', fontSize: 14.5, padding: '7px 8px', borderRadius: 9 }}
            ...
            {isRecruiter ? (
              <button onClick={openCompose} {...primaryHover} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 16, padding: 11, borderRadius: 'var(--r-ctl)', background: 'var(--primary)', color: 'var(--on-primary)', fontWeight: 600, fontSize: 15, cursor: 'pointer', border: 'none', transition: 'background .18s ease' }}>Email {name}</button>
            ) : (
              <a href={contactHref(cs)} {...primaryHover} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 16, padding: 11, borderRadius: 'var(--r-ctl)', background: 'var(--primary)', color: 'var(--on-primary)', fontWeight: 600, fontSize: 15, textDecoration: 'none', transition: 'background .18s ease' }}>Email {name}</a>
            )}
          </div>
        </aside>
      </div>
      ...
      {compose.open && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 400, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          ...
          <div style={{ ...card, position: 'relative', width: 'min(560px,100%)', maxHeight: '90vh', overflow: 'auto', padding: 24 }}>
            <h2 style={{ margin: 0, fontSize: 20, fontWeight: 700 }}>Email {name}</h2>
            <p style={{ margin: '6px 0 16px', fontSize: 14, color: 'var(--text-muted)' }}>
              Replies go directly to your email.
            </p>
            <label style={{ display: 'block', fontSize: 13.5, fontWeight: 600, marginBottom: 5, color: 'var(--text-muted)' }}>Subject</label>
            <input
              value={compose.subject}
              onChange={(e) => setCompose((c) => ({ ...c, subject: e.target.value }))}
              style={{ width: '100%', padding: '10px 12px', borderRadius: 'var(--r-ctl)', border: '1px solid var(--border-strong)', background: 'var(--bg-2)', color: 'var(--text)', fontSize: 15, outline: 'none', marginBottom: 14 }}
            />
            <label style={{ display: 'block', fontSize: 13.5, fontWeight: 600, marginBottom: 5, color: 'var(--text-muted)' }}>Message</label>
            <textarea
              value={compose.body}
              onChange={(e) => setCompose((c) => ({ ...c, body: e.target.value }))}
              rows={8}
              style={{ width: '100%', padding: '10px 12px', borderRadius: 'var(--r-ctl)', border: '1px solid var(--border-strong)', background: 'var(--bg-2)', color: 'var(--text)', fontSize: 15, outline: 'none', resize: 'vertical' }}
            />
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 18 }}>
              <button onClick={() => setCompose((c) => ({ ...c, open: false }))} disabled={sending} style={{ padding: '10px 16px', borderRadius: 'var(--r-ctl)', background: 'var(--surface-2)', color: 'var(--text)', fontWeight: 550, fontSize: 15, cursor: 'pointer', border: '1px solid var(--border)' }}>Cancel</button>
              <button onClick={sendCompose} disabled={sending} style={{ padding: '10px 18px', borderRadius: 'var(--r-ctl)', background: 'var(--primary)', color: 'var(--on-primary)', fontWeight: 600, fontSize: 15, cursor: 'pointer', border: 'none', opacity: sending ? 0.7 : 1 }}>{sending ? 'Sending…' : 'Send email'}</button>
            </div>
          </div>
        </div>
      )}
